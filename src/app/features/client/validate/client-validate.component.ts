...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, RouterLink, ButtonComponent, RatingComponent, AvatarComponent, TranslatePipe],
  template: `
      ...
      <header class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <a routerLink="/client/history" class="inline-flex items-center text-sm font-medium text-gray-500 hover:text-accent-700 transition-colors duration-200 mb-2">
            ...
            {{ 'client.validate.backToHistory' | translate }}
          </a>
          <h1 class="text-2xl font-bold text-gray-900 tracking-tight">{{ 'client.validate.title' | translate }}</h1>
          <p class="text-gray-500 text-sm mt-1">{{ 'client.validate.subtitle' | translate }}</p>
        </div>
      </header>
      ...
      @if (demandQuery.isPending()) {
        ...
        <section class="bg-white rounded-3xl shadow-soft-sm overflow-hidden">
          <div class="p-6 border-b border-gray-100">
            <h2 class="text-lg font-bold text-gray-900 mb-4">{{ 'client.validate.resultHeading' | translate }}</h2>
            <!-- Mock Before / After Gallery -->
            <div class="grid grid-cols-2 gap-4">
              <div class="space-y-2">
                <div class="aspect-[4/3] bg-gray-100 rounded-2xl overflow-hidden relative group">
                  <img src="https://images.unsplash.com/photo-1550529815-46eb132223a5?q=80&w=800&auto=format&fit=crop" class="w-full h-full object-cover grayscale opacity-80" [attr.alt]="'client.validate.beforeLabel' | translate">
                  <div class="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                    <span class="text-white font-bold tracking-wider uppercase text-sm">{{ 'client.validate.beforeLabel' | translate }}</span>
                  </div>
                </div>
              </div>
              <div class="space-y-2">
                <div class="aspect-[4/3] bg-gray-100 rounded-2xl overflow-hidden relative group">
                  <img src="https://images.unsplash.com/photo-1601362840469-51e4d8d58785?q=80&w=800&auto=format&fit=crop" class="w-full h-full object-cover" [attr.alt]="'client.validate.afterLabel' | translate">
                  <div class="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                    <span class="text-white font-bold tracking-wider uppercase text-sm">{{ 'client.validate.afterLabel' | translate }}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="p-6 bg-surface-2 flex items-center justify-between">
            <div>
              <p class="text-sm text-gray-500 mb-1">{{ 'client.validate.washedVehicleLabel' | translate }}</p>
              <p class="font-bold text-gray-900 uppercase">{{ demand()!.vehicleType }}</p>
            </div>
            <div class="text-right">
              <p class="text-sm text-gray-500 mb-1">{{ 'client.validate.amountLabel' | translate }}</p>
        ...
        <section class="bg-white rounded-3xl shadow-soft-md p-8 md:p-10 text-center max-w-xl mx-auto mt-8">
          ...
          <div class="flex flex-col items-center mb-6">
            ...
            <h3 class="text-lg font-bold text-gray-900">{{ serviceQuestion() }}</h3>
            <p class="text-gray-500 text-sm mt-1">{{ 'client.validate.ratingHelpText' | translate }}</p>
          </div>
          ...
          <form [formGroup]="form" (ngSubmit)="submitReview()" class="space-y-6">
            ...
            <div *ngIf="form.get('rating')?.touched && form.get('rating')?.invalid" class="text-red-500 text-sm mt-2">
              {{ 'client.validate.ratingRequired' | translate }}
            </div>
            ...
            <div>
              <textarea
                ...
                [placeholder]="'client.validate.commentPlaceholder' | translate"
              ></textarea>
            </div>
            ...
            <div class="pt-2">
              <app-button
                ...
                {{ 'client.validate.submitButton' | translate }}
              </app-button>
            </div>
          </form>
...
export class ClientValidateComponent {
  ...
  serviceQuestion = computed(() =>
    this.i18n.t('client.validate.serviceQuestion', {
      name: this.demand()?.washer?.name || this.i18n.t('client.validate.defaultWasherName')
    })
  );
  ...
  rateMutation = injectMutation(() => ({
    ...
    onSuccess: () => {
      this.toast.show('success', this.i18n.t('client.validate.toastReviewSuccess'));
      this.queryClient.invalidateQueries({ queryKey: ['demands'] });
      this.router.navigate(['/client/history']);
    },
    onError: () => {
      this.toast.show('error', this.i18n.t('client.validate.toastReviewError'));
    }
  }));
